...
class SPAStaticFiles(StaticFiles):
    """Static files for single-page applications."""
...
class RedirectMiddleware(BaseHTTPMiddleware):
    """Middleware to redirect the watch path to the root path with the video ID."""
...
async def commit_session_after_request(request: Request, call_next):
    """Commit the session after each request."""
    response = await call_next(request)
    Session.commit()
    return response
...
async def inspect_websocket(request: Request, call_next):
    """Inspect WebSocket upgrade requests."""
    if (
...
api.include_router(configs.router, prefix="/configs", tags=["configs"])
api.include_router(auths.router, prefix="/auths", tags=["auths"])
api.include_router(users.router, prefix="/users", tags=["users"])
api.include_router(channels.router, prefix="/channels", tags=["channels"])
api.include_router(chats.router, prefix="/chats", tags=["chats"])
api.include_router(models.router, prefix="/models", tags=["models"])
api.include_router(knowledge.router, prefix="/knowledge", tags=["knowledge"])
api.include_router(prompts.router, prefix="/prompts", tags=["prompts"])
api.include_router(tools.router, prefix="/tools", tags=["tools"])
api.include_router(memories.router, prefix="/memories", tags=["memories"])
api.include_router(folders.router, prefix="/folders", tags=["folders"])
api.include_router(groups.router, prefix="/groups", tags=["groups"])
api.include_router(files.router, prefix="/files", tags=["files"])
api.include_router(functions.router, prefix="/functions", tags=["functions"])
api.include_router(evaluations.router, prefix="/evaluations", tags=["evaluations"])
api.include_router(utils.router, prefix="/autils", tags=["utils"])
...
async def get_opensearch_xml(config: ConfigDep):  # noqa: D103
    xml_content = j2.env.get_template("opensearch.xml.j2").render(
        WEBUI_NAME=env.WEBUI_NAME,
...
@app.get("/health")
async def healthcheck():  # noqa: D103
    return {"status": True}


@app.get("/health/db")
async def healthcheck_with_db():  # noqa: D103
