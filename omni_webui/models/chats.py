...
from omni_webui.internal.db import Base, get_db
from omni_webui.models.tags import TagModel, Tags
...
class ChatTable:
    ...
    def get_chat_title_id_list_by_user_id(
        ...
        with get_db() as db:
            query = db.query(Chat).filter_by(user_id=user_id).filter_by(folder_id=None)
            query = query.filter(or_(not_(Chat.pinned), Chat.pinned.is_(None)))
    ...
    def get_chats_by_user_id_and_search_text(
        ...
        with get_db() as db:
            ...
            if not include_archived:
                query = query.filter(not_(Chat.archived))
    ...
    def get_chat_list_by_user_id_and_tag_name(
        ...
        with get_db() as db:
            ...
            logger.info(db.bind.dialect.name)
            ...
            all_chats = query.all()
            logger.info("all_chats", all_chats)
            return [ChatModel.model_validate(chat) for chat in all_chats]
    ...
    def count_chats_by_tag_name_and_user_id(self, tag_name: str, user_id: str) -> int:
        with get_db() as db:  # Assuming `get_db()` returns a session object
            ...
            logger.info(f"Count of chats for tag '{tag_name}':", count)
