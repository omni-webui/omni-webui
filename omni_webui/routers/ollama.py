...
from omni_webui._exc import ModelNotFoundError
...
async def update_config(
    *,
    ...
    config.ollama.enable = form_data.ENABLE_OLLAMA_API or False
...
@cached(ttl=3)
async def get_all_models(request: Request, config: ConfigDepends):
...
async def get_ollama_versions(
    *, request: Request, url_idx: Optional[int] = None, config: ConfigDepends
):
    if config.ollama.enable:
        if url_idx is None:
...
async def get_ollama_loaded_models(
    request: Request, config: ConfigDepends, user=Depends(get_verified_user)
):
    """
...
async def push_model(
    ...
    if url_idx is None:
        ...
        if form_data.name in models:
            ...
            raise ModelNotFoundError(form_data.name)
...
async def copy_model(
    ...
    if url_idx is None:
        ...
        if form_data.source in models:
            ...
            raise ModelNotFoundError(form_data.source)
...
async def embed(
    ...
    if url_idx is None:
        ...
        if model in models:
            ...
            raise ModelNotFoundError(form_data.model)
