...
from omni_webui.env import WEBUI_SESSION_COOKIE_SAME_SITE, WEBUI_SESSION_COOKIE_SECURE
from omni_webui.models.auths import Auths
from omni_webui.models.groups import GroupModel, Groups, GroupUpdateForm
from omni_webui.models.users import Users
from omni_webui.settings import SettingsDepends
from omni_webui.utils.auth import create_token, get_password_hash
...
class OAuthManager:
    ...
    async def handle_callback(
        self, provider, request, response, settings: SettingsDepends
    ):
        if provider not in OAUTH_PROVIDERS:
            raise HTTPException(404)
        client = self.get_client(provider)
