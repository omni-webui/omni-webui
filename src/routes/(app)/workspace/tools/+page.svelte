...
import ManifestModal from "$lib/components/workspace/Tools/ToolMenu.svelte";
import { WEBUI_NAME, tools as _tools, config, user } from "$lib/stores";
...
const deleteHandler = async (tool) => {
	...
	if (res) {
		toast.success($i.t("Tool deleted successfully"));
                    ...
                    <div class=" flex-1 self-center">
                        ...
                        <div class="px-0.5">
                            ...
                            <div class="text-xs text-gray-500 shrink-0">
                                <Tooltip
                                    content={tool?.user?.email ?? $i.t('Deleted User')}
                                    className="flex shrink-0"
                                    placement="top-start"
                                >
                                    {$i.t('By {{name}}', {
                                        name: capitalizeFirstLetter(
                                            tool?.user?.name ?? tool?.user?.email ?? $i.t('Deleted User')
                                        )
                                    })}
                                </Tooltip>
                            </div>
...
{#if $user?.role === 'admin'}
    <div class=" flex justify-end w-full mb-2">
        <div class="flex space-x-2">
            ...
            >
                <div class=" self-center mr-2 font-medium line-clamp-1">{$i.t('Import Tools')}</div>
            ...
            >
                <div class=" self-center mr-2 font-medium line-clamp-1">{$i.t('Export Tools')}</div>
...
{#if $config?.features.enable_community_sharing}
    <div class=" my-16">
        ...
        >
            <div class=" self-center">
                <div class=" font-semibold line-clamp-1">{$i.t('Discover a tool')}</div>
                <div class=" text-sm line-clamp-1">
                    {$i.t('Discover, download, and explore custom tools')}
                </div>
            </div>
...
<DeleteConfirmDialog
    ...
    title={$i.t('Delete tool?')}
    on:confirm={() => {
        deleteHandler(selectedTool);
    }}
>
    <div class=" text-sm text-gray-500">
        {$i.t('This will delete')} <span class="  font-semibold">{selectedTool.name}</span>.
    </div>
</DeleteConfirmDialog>
...
<ConfirmDialog
    ...
    on:confirm={() => {
        ...
        reader.onload = async (event) => {
            ...
            toast.success($i.t('Tool imported successfully'));
...
>
    <div class="text-sm text-gray-500">
        <div class=" bg-yellow-500/20 text-yellow-700 dark:text-yellow-200 rounded-lg px-4 py-3">
            <div>{$i.t('Please carefully review the following warnings:')}</div>

            <ul class=" mt-1 list-disc pl-4 text-xs">
                <li>
                    {$i.t('Tools have a function calling system that allows arbitrary code execution')}.
                </li>
                <li>{$i.t('Do not install tools from sources you do not fully trust.')}</li>
            </ul>
        </div>
