...
export const removeLastWordFromString = (
	inputString: string,
	wordString: string,
) => {
...
const convertOpenAIMessages = (convo) => {
	...
	for (const obj of messages) history[obj.id] = obj;
...
export const titleGenerationTemplate = (
	...
	template = template.replace(
		...
		(match, startLength, endLength, middleLength) => {
			...
			if (match.startsWith("{{prompt:start:")) {
				return prompt.substring(0, startLength);
			}
			if (match.startsWith("{{prompt:end:")) {
				return prompt.slice(-endLength);
			}
			if (match.startsWith("{{prompt:middletruncate:")) {
...
export const createMessagesList = (history, messageId: string | null) => {
	if (messageId === null) {
		return [];
	}
