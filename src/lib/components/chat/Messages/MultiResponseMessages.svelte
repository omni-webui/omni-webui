...
let parentMessage: OmniWebUI.Message;
let groupedMessageIds: Record<string, { messageIds: string[] }> = {};
...
const showPreviousMessage = async (modelIdx: string) => {
...
const showNextMessage = async (modelIdx: string) => {
...
const mergeResponsesHandler = async () => {
	...
	await mergeResponses(messageId, responses, chatId);
};
