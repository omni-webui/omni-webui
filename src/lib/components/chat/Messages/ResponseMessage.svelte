...
let message: OmniWebUI.Message = JSON.parse(
...
const i = getContext<Writable<i18n>>("i18n");
...
const copyToClipboard = async (text: string) => {
	...
	if (res) {
		toast.success($i.t("Copying to clipboard was successful!"));
	}
};
...
const toggleSpeakMessage = async () => {
	...
	if (!(message?.content ?? "").trim().length) {
		toast.info($i.t("No content to speak"));
		return;
	}
...
const generateImage = async (message: OmniWebUI.Message) => {
	generatingImage = true;
	const res = await imageGenerations(localStorage.token, message.content).catch(
		(error) => {
			toast.error(error);
		},
	);
...
const feedbackHandler = async (
	...
	if (message?.feedbackId) {
		...
		feedback = await createNewFeedback(localStorage.token, feedbackItem).catch(
			(error) => {
				toast.error(error);
			},
		);
		...
		<div class={`flex-shrink-0 ${($settings?.chatDirection ?? 'LTR') === 'LTR' ? 'mr-3' : 'ml-3'}`}>
			<ProfileImage
				src={model?.info?.meta?.profile_image_url ??
					($i.language === 'dg-DG' ? `/doge.png` : `${WEBUI_BASE_URL}/static/favicon.png`)}
				className={'size-8'}
			/>
		</div>
		...
		<div class="flex-auto w-0 pl-1">
			<Name>
				...
				{#if message.timestamp}
					...
					>
						{dayjs(message.timestamp * 1000).format($i.t('h:mm a'))}
					</span>
				{/if}
			</Name>
					...
					<div>
								...
								<div class="status-description flex items-center gap-2 py-0.5">
											...
											<div class="flex flex-col justify-center -space-y-0.5">
												...
												>
													...
													{:else if status?.description === 'No search query generated'}
														{$i.t('No search query generated')}
													{:else if status?.description === 'Generating search query'}
														{$i.t('Generating search query')}
													{:else}
														{status?.description}
													{/if}
												</div>
									...
									{:else if status?.action === 'knowledge_search'}
										<div class="flex flex-col justify-center -space-y-0.5">
											...
											>
												{$i.t(`Searching Knowledge for "{{searchQuery}}"`, {
						...
						{:else}
							<div class="w-full flex flex-col relative" id="response-content-container">
								...
								{#if message?.error}
									<MessageError content={message?.error?.content ?? message.content} />
								{/if}
							...
							{#if message.done}
								{#if !readOnly}
									{#if $user.role === 'user' ? ($user?.permissions?.chat?.edit ?? true) : true}
										<Tooltip content={$i.t('Edit')} placement="bottom">
											<button
												class="{isLastMessage
													? 'visible'
								...
								<Tooltip content={$i.t('Copy')} placement="bottom">
									<button
										class="{isLastMessage
											? 'visible'
								...
								<Tooltip content={$i.t('Read Aloud')} placement="bottom">
									<button
								...
								{#if $config?.features.enable_image_generation && !readOnly}
									<Tooltip content={$i.t('Generate Image')} placement="bottom">
										<button
											class="{isLastMessage
												? 'visible'
												: 'invisible group-hover:visible'}  p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition"
								...
								{#if !readOnly}
									{#if $config?.features.enable_message_rating ?? true}
										<Tooltip content={$i.t('Good Response')} placement="bottom">
											<button
												class="{isLastMessage
													? 'visible'
										...
										<Tooltip content={$i.t('Bad Response')} placement="bottom">
											<button
												class="{isLastMessage
													? 'visible'
													: 'invisible group-hover:visible'} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg {(
									...
									{#if isLastMessage}
										<Tooltip content={$i.t('Continue Response')} placement="bottom">
											<button
												type="button"
									...
									<Tooltip content={$i.t('Regenerate')} placement="bottom">
										<button
											type="button"
											class="{isLastMessage
												? 'visible'
