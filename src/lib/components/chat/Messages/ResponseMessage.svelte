...
export let showPreviousMessage: (message?: Message) => void;
export let showNextMessage: (message?: Message) => void;
export let rateMessage: (id: string, rating: 1 | -1) => void;

export let copyToClipboard: (text: string) => Promise<boolean | void>;
		...
		if (message.info.openai) {
			...
			tooltipContent = `response_token/s: ${
				Math.round(
                    ...
                    prompt_eval_count: ${message.info.prompt_eval_count}<br/>
                    prompt_eval_duration: ${
											Math.round(((message.info.prompt_eval_duration ?? 0) / 1000000) * 100) / 100
										}ms<br/>
                    eval_count: ${message.info.eval_count}<br/>
                    eval_duration: ${
