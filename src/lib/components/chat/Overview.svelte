...
const drawFlow = async () => {
	...
	Object.keys(history.messages).forEach((id) => {
		...
		const level = message.parentId
			? (positionMap.get(message.parentId)?.level ?? -1) + 1
			: 0;
	...
	Object.keys(history.messages).forEach((id) => {
		...
		if (parentId) {
			edgeList.push({
				...
				type: "smoothstep",
				animated:
					history.currentId === id || recurseCheckChild(id, history.currentId),
			});
		}
	});
...
const recurseCheckChild = (nodeId, currentId) => {
	...
	return node.childrenIds?.some(
		(id) => id === currentId || recurseCheckChild(id, currentId),
	);
};
