...
import { convertMessagesToHistory, copyToClipboard, promptTemplate, splitStream } from '$lib/utils';
...
const getWebSearchResults = async (model: string, parentId: string, responseId: string) => {
	...
	responseMessage.status = {
		...responseMessage.status,
		description: $i18n.t("Searching the web for '{{searchQuery}}'", {
			searchQuery
		})
	};
	messages = messages;
	...
	if (results) {
		...
		if (responseMessage?.files === undefined) {
...
const generateChatTitle = async (userPrompt) => {
	if ($settings?.title?.auto ?? true) {
		...
		const title = await generateTitle(
			...
			userPrompt,
			$chatId,
			titleModel?.owned_by === 'openai' ? `${OPENAI_API_BASE_URL}` : `${OLLAMA_API_BASE_URL}/v1`
		);
...
const generateChatSearchQuery = async (modelId: string, prompt: string) => {
	...
	return await generateSearchQuery(
		...
		previousMessages,
		prompt,
		taskModel?.owned_by === 'openai' ? `${OPENAI_API_BASE_URL}` : `${OLLAMA_API_BASE_URL}/v1`
	);
};
