...
$: visionCapableModels = [
	...
	(model) =>
		$models.find((m) => m.id === model)?.info?.meta?.capabilities?.vision ??
		true,
);
...
const uploadFileHandler = async (file, fullContext = false) => {
	if (
		...
		toast.error($i.t("You do not have permission to upload files."));
		return null;
	}
	...
	if (fileItem.size === 0) {
		toast.error($i.t("You cannot upload an empty file."));
		return null;
	}
	...
	if (
		["audio/mpeg", "audio/wav", "audio/ogg", "audio/x-m4a"].includes(file.type)
	) {
...
const inputFilesHandler = async (inputFiles: File[]) => {
	...
	for (const file of inputFiles) {
		...
		if (
			...
			toast.error(
				$i.t("File size should not exceed {{maxSize}} MB.", {
		...
		if (
			...
			if (visionCapableModels.length === 0) {
				toast.error($i.t("Selected model(s) do not support image inputs"));
				return;
			}
							...
							{#if webSearchEnabled}
								<div class="flex items-center justify-between w-full">
									<div class="flex items-center gap-2.5 text-sm dark:text-gray-500">
										...
										<div class=" translate-y-[0.5px]">{$i.t('Search the web')}</div>
									</div>
								</div>
							{/if}
					...
					<input
						...
						on:change={async () => {
							if (inputFiles && inputFiles.length > 0) {
								...
								toast.error($i.t(`File not found.`));
							}
								...
								<div class=" flex">
										...
										<InputMenu
											...
											uploadGoogleDriveHandler={async () => {
												try {
													...
													toast.error(
														$i.t('Error accessing Google Drive: {{error}}', {
															error: error.message
														})
													);
												}
									...
									{#if $settings?.richTextInput ?? true}
										...
										>
											<RichTextInput
												...
												placeholder={placeholder ? placeholder : $i.t('Send a Message')}
												largeTextAsFile={$settings?.largeTextAsFile ?? false}
												autocomplete={true}
												generateAutoCompletion={async (text) => {
													if (selectedModelIds.length === 0 || !selectedModelIds.at(0)) {
														toast.error($i.t('Please select a model first.'));
													}
									...
									<div class="self-end mb-1.5 flex space-x-1 mr-1">
										{#if !history?.currentId || history.messages[history.currentId]?.done == true}
											<Tooltip content={$i.t('Record voice')}>
												<button
													...
													on:click={async () => {
														try {
															...
															toast.error($i.t('Permission denied when accessing microphone'));
														}
													}}
										...
										{#if !history.currentId || history.messages[history.currentId]?.done == true}
														...
														<button
															...
															on:click={async () => {
																if (selectedModels.length > 1) {
																	toast.error($i.t('Select only one model to call'));
																...
																try {
																	...
																	toast.error(
																		$i.t('Permission denied when accessing media devices')
																	);
																}
															}}
															aria-label="Call"
											...
											{:else}
												<div class=" flex items-center">
													<Tooltip content={$i.t('Send message')}>
														<button
