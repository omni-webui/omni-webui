...
const createFolder = async (_name = "Untitled") => {
	let name = _name;
	if (name === "") {
		toast.error($i.t("Folder name cannot be empty."));
		return;
	}
	...
	for (const file of files) {
		...
		reader.onload = async (e) => {
			...
			try {
				...
				toast.error($i.t("Invalid file format."));
			}
		};
	...
	>
			...
			<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
				...
				>
					...
					<div class="flex self-center">
						<div class=" self-center font-medium text-sm font-primary">{$i.t('Workspace')}</div>
					</div>
				</a>
			</div>
		{/if}
		...
		<div class="relative {$temporaryChatEnabled ? 'opacity-20' : ''}">
			...
			<div class="absolute z-40 right-3.5 top-1">
				<Tooltip content={$i.t('New folder')}>
					<button
