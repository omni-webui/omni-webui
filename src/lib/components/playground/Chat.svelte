...
const chatCompletionHandler = async () => {
	...
	let responseMessage: { role: string; content: string };
	if (messages.at(-1)?.role === "assistant") {
		responseMessage = messages.at(-1) as { role: string; content: string };
	} else {
		responseMessage = {
			role: "assistant",
			content: "",
					...
					<div slot="content">
						<div class="pt-1 px-1.5">
							<textarea
								...
								bind:value={system}
								placeholder={$i.t("You're a helpful assistant.")}
								rows="4"
							/>
						</div>
					</div>
				...
				<div class="border border-gray-50 dark:border-gray-850 w-full px-3 py-2.5 rounded-xl">
					<div class="py-0.5">
						<textarea
							bind:value={message}
							class=" w-full h-full bg-transparent resize-none outline-none text-sm"
							placeholder={$i.t(`Enter {{role}} message here`, {
								role: role === 'user' ? $i.t('a user') : $i.t('an assistant')
							})}
							on:input={(e) => {
