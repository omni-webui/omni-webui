...
const saveHandler = async () => {
	...
	if (config?.comfyui?.COMFYUI_WORKFLOW) {
		config.comfyui.COMFYUI_WORKFLOW_NODES = requiredWorkflowNodes.map(
			(node) => {
				return {
					type: node.type,
					key: node.key,
...
onMount(async () => {
	if ($user.role === "admin") {
		...
		requiredWorkflowNodes = requiredWorkflowNodes.map((node) => {
			const n =
				config.comfyui.COMFYUI_WORKFLOW_NODES.find(
					(n) => n.type === node.type,
				) ?? node;
			...
			return {
				...
				key: n.key,
				node_ids:
					typeof n.node_ids === "string" ? n.node_ids : n.node_ids.join(","),
			};
		});

		const imageConfigRes = await getImageGenerationConfig(
			localStorage.token,
		).catch((error) => {
			toast.error(error);
			return null;
