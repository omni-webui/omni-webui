...
const pullModelHandler = async () => {
	...
	if ($MODEL_DOWNLOAD_POOL[sanitizedModelTag]) {
		toast.error(
			$i.t(`Model '{{modelTag}}' is already in queue for downloading.`, {
	...
	if (res) {
		...
		if ($MODEL_DOWNLOAD_POOL[sanitizedModelTag].done) {
			toast.success(
				$i.t(`Model '{{modelName}}' has been successfully downloaded.`, {
			...
			toast.error($i.t("Download canceled"));
		}
...
const uploadModelHandler = async () => {
	...
	if (modelUploadMode === "file") {
		...
		if (file) {
			...
			fileResponse = await uploadModel(localStorage.token, file, urlIdx).catch(
				(error) => {
					toast.error(error);
					return null;
				},
...
const deleteModelHandler = async () => {
	...
	if (res) {
		toast.success($i.t("Deleted {{deleteModelTag}}", { deleteModelTag }));
	}
						...
						<div class="space-y-2">
							...
							<div>
								<div class=" mb-2 text-sm font-medium">
									{$i.t('Pull a model from Ollama.com')}
								</div>
								<div class="flex w-full">
									<div class="flex-1 mr-2">
										<input
											class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-none"
											placeholder={$i.t('Enter model tag (e.g. {{modelTag}})', {
												modelTag: 'mistral:7b'
											})}
											bind:value={modelTag}
										/>
								...
								<div class="mt-2 mb-1 text-xs text-gray-400 dark:text-gray-500">
									{$i.t('To access the available model names for downloading,')}
									<a
										class=" text-gray-500 dark:text-gray-300 font-medium underline"
										href="https://ollama.com/library"
										target="_blank">{$i.t('click here.')}</a
									>
								</div>
							...
							<div>
								<div class=" mb-2 text-sm font-medium">{$i.t('Delete a model')}</div>
								<div class="flex w-full">
									<div class="flex-1 mr-2">
										<select
											class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-none"
											bind:value={deleteModelTag}
											placeholder={$i.t('Select a model')}
										>
											{#if !deleteModelTag}
												<option value="" disabled selected>{$i.t('Select a model')}</option>
											{/if}
							...
							<div>
								<div class=" mb-2 text-sm font-medium">{$i.t('Create a model')}</div>
								<div class="flex w-full">
							...
							<div class="pt-1">
								<div class="flex justify-between items-center text-xs">
									<div class=" text-sm font-medium">{$i.t('Experimental')}</div>
									<button
										class=" text-xs font-medium text-gray-500"
										type="button"
										on:click={() => {
											showExperimentalOllama = !showExperimentalOllama;
										}}>{showExperimentalOllama ? $i.t('Hide') : $i.t('Show')}</button
									>
								</div>
							</div>
							...
							{#if showExperimentalOllama}
								...
								>
									<div class=" mb-2 flex w-full justify-between">
										<div class="  text-sm font-medium">{$i.t('Upload a GGUF model')}</div>
										...
										>
											{#if modelUploadMode === 'file'}
												<span class="ml-2 self-center">{$i.t('File Mode')}</span>
											{:else}
												<span class="ml-2 self-center">{$i.t('URL Mode')}</span>
											{/if}
										</button>
									</div>
											...
											{:else}
												<div class="flex-1 {modelFileUrl !== '' ? 'mr-2' : ''}">
													<input
														...
														placeholder={$i.t('Type Hugging Face Resolve (Download) URL')}
													/>
												</div>
											{/if}
										</div>
									...
									<div class=" mt-1 text-xs text-gray-400 dark:text-gray-500">
										{$i.t('To access the GGUF models available for downloading,')}
										<a
											class=" text-gray-500 dark:text-gray-300 font-medium underline"
									...
									{#if uploadMessage}
										<div class="mt-2">
											<div class=" mb-2 text-xs">{$i.t('Upload Progress')}</div>
