...
const rankHandler = async (similarities: Map<string, number> = new Map()) => {
	...
	rankedModels = $models
		.filter(
			(m) =>
				m?.owned_by !== "arena" && (m?.info?.meta?.hidden ?? false) !== true,
		)
		.map((model) => {
...
function calculateModelStats(
	...
	function calculateEloChange(
		...
		const expectedScore = 1 / (1 + 10 ** ((ratingB - ratingA) / 400));
...
const loadEmbeddingModel = async () => {
	...
	const allTags = new Set(
		feedbacks.flatMap((feedback) => feedback.data.tags || []),
	);
